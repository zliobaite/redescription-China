...
DEF_SETTS = {"map_proj": "mill", "map_resolution": "crude", "sea_color": "#F9FCFF",
             "map_elem_area": ["seas"], "map_elem_natural": ["coasts"], "map_elem_geop": ["countries"]}
...
class MapBase:
    ...
    def getBasemapProjSetts(tcl, prefs):
        ...
        if "map_proj" in prefs:
            tpro = re.sub(" *\(.*\)$", "", prefs["map_proj"])
            if tpro in tcl.proj_names:
                proj = tcl.proj_names[tpro]
            elif tpro in tcl.proj_names.values():
                proj = tpro
        resolution = "c" ## crude, low, intermediate, high, full
        if "map_resolution" in prefs:
            resolution = prefs["map_resolution"][0]
    ...
    def getBasemapBackSetts(tcl, prefs):
        ...
        for typ_elem in ["map_elems", "map_elem_area", "map_elem_natural", "map_elem_geop", "map_elem_circ"]:
            if typ_elem in prefs:
                for elem in prefs[typ_elem]:                    
                    draws[elem] = True
                    
        for k in ["map_back_alpha", "map_back_scale"]:
            if k in prefs:
                more[k] = prefs[k]/100.0
            else:
                more[k] = 1.
        for k in ["map_back"]:
            if k in prefs:
        ...
        for color_k in colors.keys():
            if color_k in prefs:
                # colors[color_k] = "#"+"".join([ v.replace("x", "")[-2:] for v in map(hex, prefs[color_k])])
                colors[color_k] = prefs[color_k] 
    ...
    @classmethod
    def makeBasemapBack(tcl, prefs, bm_args, bm=None, axe=None):
        if bm is None:
            return
        ...
        if more.get("map_back") in func_map:
            ...
            try:
                func_map[more.get("map_back")](alpha=more["map_back_alpha"], scale=more["map_back_scale"], ax=axe) #, zorder=tcl.background_zorder)
                bd = True
            except IndexError:
                bd = False
        ...
        if not bd:
            if bounds_color != colors["none"] or sea_color != colors["none"]:
                bm.drawmapboundary(color=bounds_color, fill_color=sea_color, ax=axe)#, zorder=tcl.background_zorder)
            if contin_color != colors["none"] or lake_color != colors["none"] or sea_color != colors["none"]:
                bm.fillcontinents(color=contin_color, lake_color=lake_color, ax=axe)#, zorder=tcl.background_zorder)
            # bm.drawlsmask(land_color=contin_color,ocean_color=sea_color,lakes=draws["lakes"])
        
        if draws["rivers"]:
            bm.drawrivers(color=colors["sea_color"], zorder=tcl.background_zorder, ax=axe)
        if draws["coasts"]:
            bounds_color = colors["line_color"]
            bm.drawcoastlines(color=colors["line_color"], linewidth=0.2, zorder=tcl.background_zorder, ax=axe)
        if draws["countries"]:
            bounds_color = colors["line_color"]
            bm.drawcountries(color=colors["line_color"], zorder=tcl.background_zorder, ax=axe)
        if draws["states"]:
            bounds_color = colors["line_color"]
            bm.drawstates(color=colors["line_color"], zorder=tcl.background_zorder, ax=axe)
            
        if draws["parallels"]:
            tt = tcl.getParallelsRange(bm_args)
            # print("parallels", tt)
            bm.drawparallels(tt, linewidth=0.5, labels=[1,0,0,1], zorder=tcl.background_zorder, ax=axe)
        if draws["meridians"]:
            tt = tcl.getMeridiansRange(bm_args)
            # print("meridians", tt)
            bm.drawmeridians(tt, linewidth=0.5, labels=[0,1,1,0], zorder=tcl.background_zorder, ax=axe)



                
def prepare_map(prefs, corners, fig=None, axe=None):
                
    # MapBase.background_zorder = -1    
    llon, ulon, llat, ulat = corners
    bm, bm_args = MapBase.makeBasemapProj(prefs, corners)    
    ...
    if axe is None:
        ...
        axe.set_xlim([midlon-1.05*mside, midlon+1.05*mside])
        axe.set_ylim([midlat-1.05*mside, midlat+1.05*mside])
    MapBase.makeBasemapBack(prefs, bm_args, bm, axe=axe)
    return axe, bm, bm_args
        
    # def getPosInfo(bm, x, y):
    #     return bm(x, y, inverse=True)
    # def getCoordXYtoP(bm, x, y):
    #     return bm(x, y)
