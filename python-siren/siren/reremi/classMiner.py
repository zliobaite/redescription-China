...
class ExpMiner(object):
    ...
    def expandRedescriptionsTree(self, nextge, partial, final=None):
        ...
        self.logger.printL(1, {"partial":partial["batch"]}, 'result', self.ppid)
        self.logger.printL(1, "%d redescriptions [P]" % len(partial["results"]), 'status', self.ppid)
...
################################
    ...
    def full_run(self, cust_params={}):
        ...
        while initial_red is not None and self.questionLive():
            ...
            self.logger.printL(1, "%d redescriptions [F]" % len(self.final["results"]), 'status', self.id)
            for i in self.final["results"]:
                self.logger.printL(6, "--- %s" % self.final["batch"][i])               
